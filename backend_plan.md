...
```mermaid
flowchart TD
    P1[Phase 1: Auth] --> P2[Phase 2: Event/Session model]
    P7[Phase 7: PPT pipeline] --> |independent, no blocking deps| DONE7((can start day 1))
    P2 --> P3[Phase 3: Form builder]
    P2 --> P4[Phase 4: Realtime DO]
    P3 --> P6[Phase 6: Export/Analytics]
    P4 --> P5[Phase 5: Participant UX]
    P4 --> P6
    P5 --> P6
    P1 & P2 & P3 & P4 & P5 & P6 & P7 --> P8[Phase 8: Hardening]

```

**Recommended parallel tracks for multiple agents/engineers:**


| Group                                   | Phases                                                           | Rationale                                                                                                                                                                                        |
| --------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Start immediately, no dependencies**  | Phase 1 (Auth) **and** Phase 7 (PPT pipeline)                    | Auth touches users/JWT; PPT pipeline touches R2/slides. Zero schema overlap — safe to run as two separate agent sessions from day one.                                                           |
| **Once Phase 1 lands (or is stubbed)**  | Phase 2 (Event/Session model)                                    | Needs `owner_id` semantics from Phase 1, but can begin against a stub "current user = fixed test id" if you want to not block on Phase 1 finishing.                                              |
| **Once Phase 2 schema is stable**       | Phase 3 (Form builder) **and** Phase 4 (Realtime DO) in parallel | Both only need the `slides`/`sessions` tables to exist, not each other. Agree the DO message contract (§7 of architecture doc) upfront so Phase 5 can also start early against a mock.           |
| **Once Phase 4's WS contract is fixed** | Phase 5 (Participant UX)                                         | Frontend-heavy; can start against a mocked WebSocket before Phase 4's backend is 100% done, as long as the message shapes are agreed first.                                                      |
| **Once real response data exists**      | Phase 6 (Export/Analytics)                                       | Needs Phase 2 schema plus at least some data flowing from Phase 4/5 to validate exports meaningfully.                                                                                            |
| **Last, cross-cutting**                 | Phase 8 (Hardening)                                              | Touches every endpoint added by the phases above; do a full pass once the rest is functionally complete, though items like rate-limiting can be picked off incrementally as each endpoint ships. |


**Practical recommendation for a single AI agent working sequentially:** Phase 1 → Phase 7 (can genuinely interleave, no shared files) → Phase 2 → Phase 3/4 (interleave) → Phase 5 → Phase 6 → Phase 8.

**If you have 2–3 agent sessions running concurrently:** Session A does Phase 1 → 2 → 3/4 → 5 → 6; Session B does Phase 7 independently and merges in whenever; a third session can pick up Phase 8 items incrementally as soon as each phase's endpoints stabilize rather than waiting for everything.